...
if (isset($_POST['editProfile'])) {
    ...
    echo $previous;
    if ($avatar != "") {
        $query = "UPDATE `interns` SET `name`='$profile_name',`avatar`='$path', `email`='$email', `password`='$password'
        WHERE `intern_id`=$profile_id";
    } else {
        $query = "UPDATE `interns` SET `name`='$profile_name', `email`='$email', `password`='$password'
        WHERE `intern_id`=$profile_id";
    }
